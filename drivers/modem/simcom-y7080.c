...
static int parse_cgnssinfo(char *gps_buf)
{
	...
	strcpy(gnss_data.ut_time, utc_time);

	if(speed)
		gnss_data.knots = get_float_from_str(speed);

	gnss_data.hdop = get_init_from_str(hdop);
...
int mdm_y7080_stop_gnss(void)
{
	...
	ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler, NULL, 0U, "AT+CGNSSPWR=0",
			&mdata.sem_response, K_SECONDS(2));
...
static int offload_connect(void *obj, const struct sockaddr *addr, socklen_t addrlen)
{
	...
	ret = snprintk(buf, sizeof(buf), "AT+NSOCR=\ %s,\ %d, %d, %d, %d, %d", protocol_type, protocol, sock->sock_fd, dst_port,
		       receive_ctrl, addr->sa_family);
	if (ret < 0) {
		LOG_ERR("Failed to build connect command. ID: %d, FD: %d", sock->id, sock->sock_fd);
		errno = ENOMEM;
		return -1;
